...
NVOFA 不需要光流权重和迭代。初始化失败会提示并尝试 RAFT（需权重）；处理中失败停止，不中途换算法。默认仍 RAFT，启动仍关闭光流。深度推理本版暂时下线。开发从仓库根目录 `run.bat` 启动。
...
每次启动默认关闭推理，但会保留已保存的调节参数。完整组件无需另装 Python、PyTorch 或 CUDA Toolkit，不要只复制组件 EXE。
...
| 光流模型、分析长边、迭代 | RAFT-Large、512 px、6 次；NVOFA 网格默认 4×4 |
| 光流方向、精度 | 当前帧→上一帧、FP32 |
| 光流显示量程 | 3 px/帧；仅影响可视化 |
