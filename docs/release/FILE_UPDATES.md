...
维护者于 2026-09-19 将 `packaging/update-policy.json` 的支持策略定为 `previous-release`：
**每次只为上一个正式版本生成 lite/full 两个增量包**，更早版本使用整包升级，不自动串联升级链。
当前 v2.3.1 的源版本是 v2.3.0，基线为 `dist/v2.3.0-release-20260919/editions`。
该目录的 lite ZIP、full/addon 分卷已与 GitHub 正式 v2.3.0 附件 SHA256 对齐；旧候选状态记录保留为历史。
v2.2.0/v2.2.1 已退出本次支持范围，不恢复其已删除目录；v2.2.0 仍是更新器首次引入版本。
...
3. 通过 `scripts/build_release.ps1` 构建新基础包（复用 `.venv`，选择全新输出），然后使用
   ...
   完成三形态打包。**无需额外手动调用差异包脚本**：入口默认读取策略，先核验选中基线的实际文件，
   再构建完整包及选中的上一个正式版本到当前 `APP_VERSION` 的 lite/full `.dlssupdate`。
...
6. 新版完成冻结程序、组件与升级验收后，将其版本和正式 `editions` 路径加入策略 `baselines`，
   使下一次发版仅选该版作为增量源。当前目标自身的登记不影响本次上传集合复验；
   不自动将尚未验证的候选登记为基线，也不自动删除旧目录。

路径迁移时可重复传 `--update-baseline "旧版editions新位置"`（预检和三形态打包均支持）。
参数只能重定位策略选中的版本，不能拿另一版本覆盖。`previous-release` 不因文件缺失自动回退到更老基线。
...
下载后及安装前检查整个旧版受管基线，包括未变化的依赖。兼容旧用户的唯一例外是
`_internal/nvngx_dlssnr.dll`：当官方 before/after 清单中该文件完全相同时，允许本地为不同的非空普通文件，
并原位保留；不覆盖、不迁移，也不改动 `mods` 中的版本。安装完成时仍核对其与安装前的实际内容一致。
该文件缺失、为空、为目录或链接，或本次发行需更换/移除它时，不适用例外；其他受管文件的缺失或修改仍停止更新。
正式发行基线预检、下载摘要、payload、更新助手及回滚备份校验不放宽。
运行库自定义仍推荐放 `mods`。自定义外置组件目录需人工升级；新安装/移除本地组件导致形态变化也会停止。

此兼容逻辑必须同时包含在主程序和更新助手中。已安装的旧版（包括原版 v2.2.2）仍执行旧校验，
仅重新打目标版本的增量包不能修复旧客户端；这类用户当前需恢复官方旧版内置 DLL（自用版本保留到 `mods`），
或使用对应形态的新整包升级。不得修改正式旧版基线或删减增量清单来绕过旧校验。
...
2026-09-20 源码兼容性验证：上述六个测试模块加 `tests.test_release_updates` 共 99 项通过。
新增覆盖 lite/full 保留旧用户内置运行库（含与 mods 并存）、缺失/空文件/目录/硬链接拒绝、
官方运行库变化时保持严格校验、安装中变动检测及失败回滚。只读检查现有 v2.2.2 → v2.3.0
的 lite/full 增量清单，确认运行库 before/after 相同且不在 payload 中；未修改这些发行产物，未重建冻结客户端。
