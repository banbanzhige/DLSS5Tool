...
- 仅支持 **v2.3.0 → v2.3.1** 两个增量：lite 15,832,585 B，full 16,082,454 B；
  三形态打包入口与最终 `--check-upload` 通过，不包含 v2.2.2 直达增量。
- ZIP：lite 579,734,091 B；full 3,633,513,830 B；addon 3,053,782,250 B。
  full/addon 各两卷，CRC、分卷重组、全部附件 SHA256、三份 Release Notes 一致性通过。
- 新冻结主程序/助手含运行库定点兼容；原版 v2.2.2/v2.3.0 客户端仍需按升级说明处理旧校验。
  附 1,477 B 的 `DLSS5Tool-v2.3.1-addon-component-reuse.zip` 和 `UPGRADE_v2.3.1.md`，
  严格验证新 lite＋官方 v2.2.2/v2.3.0 旧组件＋说明补齐精确等于新 full。
- 完整 unittest 704 项成功（10 跳过）；冻结 lite/full、缺组件拒绝、RAFT、精确叠加通过；
  两个真实增量流式还原目标通过，未在正式旧基线上执行更新。冻结助手两组合成事务通过。
- v2.3.0 的远端正式发行附件已与本地原包摘要对齐，补登记为基线；v2.3.1 已验收的 lite/full
  目录和清单也登记，既有目录不删除。公开分发审查仍待确认（保留 LocalCandidate 标记）；
  未推送、创建 Release、上传，不宣称全套 HDR/GPU 或干净机器认证。
- 细节、边界及重建命令见 [v2.3.1 打包记录](V231_PACKAGING_20260920.md)；
  证据位于 `output/package-v231-20260920-verification/`。
- 已清理本任务可重建缓存/重复副本，磁盘实测释放约 6.07 GiB；task tmp 仅余用途记录，旧依赖/备份未动。

## v2.3.0 — 2026-09-19 · 本地候选已打包，未发布

后续状态：2026-09-20 已只读确认该版本在 GitHub 正式发布，主 ZIP 和四个分卷摘要均匹配本地；
以下“未发布”及未登记状态为当时记录，v2.3.1 已据实补登记原样基线，未改写旧产物或审查标记。

- 本地产物：`dist/v2.3.0-release-20260919/editions/`；附件集合：其下 `github-assets/`。
- 轻量 ZIP 579,726,011 B；完整 ZIP 3,633,505,750 B；附加 ZIP 3,053,782,250 B。
  完整/附加各两卷，摘要及文件清单见 `package-report.json`、`SHA256SUMS.txt`。
- 增量仅支持 **v2.2.2 → v2.3.0**：lite 231,556,118 B，full 231,806,914 B；
  `release_updates.py --check-upload` 通过，两种形态均齐全。
- 三包 CRC、精确叠加、分卷摘要及三份 Release Notes 一致性通过；冻结 lite/full、叠加安装和 RAFT 诊断通过，
  记录在 `output/package-v230-release-20260919-verification/report.json`。
- 更新助手合成安装、设置保留与备份通过。额外 HDR 冻结检查出现显存余量不足及超时，未记成功；
  候选暂不登记为正式基线，未上传。详见 [本地候选记录](V230_READINESS_20260919.md)。
- 两个真实增量包的全部变化文件与未变基线流式叠加后精确匹配目标；本轮最终 70 项定向测试通过。
  未声称干净机器或全套 GPU 验收完成。
- 收尾删除本轮构建缓存、合成助手测试副本、重复基础包及附加包暂存，实际释放 6,483,296,256 B
  （约 6.04 GiB），可用空间恢复至 145,892,188,160 B。删除物为可重建副本，不从回收站恢复；
  最终 ZIP、分卷、两个增量包、lite/full 目录和全部验证报告保留。
  本任务 tmp 剩余约 170 KiB（用途登记、四帧合成 HDR 输入与小型报告），2026-09-26 复核；
  全部 tmp 仍约 28.73 GiB，旧依赖与备份未动。发行目录按文件长度合计约 20.36 GiB（含同字节附件链接计数）。

### 构建前的决定与历史准备记录

**当前决定**：维护者明确只支持上一个正式版本，策略已改为 `previous-release`。
本次仅生成 v2.2.2 → v2.3.0 的 lite/full 两个增量包，真实基线预检及 33 项相关测试通过。
材料清单已生成（19 份源码、25 个 DLL，总计 188,864,047 B）；通过显式 `-LocalCandidate`
构建本地候选，核验材料但不伪造公开分发确认。下面多版本要求与下载失败为历史准备记录，
详见 [当前执行决定](V230_READINESS_20260919.md)。

- 本地源码提交 `23d7372`：版本统一为 2.3.0，包含本次 GPU 导出、图片序列导入、队列交互、
  预览及诊断升级；未推送、创建 Release 或上传附件。
- 材料收集脚本已随上述提交纳入版本控制，后续修复分别为 `ab458e9`（下载重试、超时及失败清理）、
  `dc971c9`（SDK 头文件内嵌许可证）、`22c8032`（zlib 官方源码地址及固定 SHA256）。
- v2.2.0、v2.2.1、v2.2.2 正式 lite/full 共六套基线已通过严格预检；支持策略未缩减。
  本次交付仍须包含三形态整包及上述三个源版本直达 v2.3.0 的六个增量包，目前均未生成。
- 材料收集定向测试 4 项通过。现有源码归档、通知及构建材料共 187,306,272 B（约 178.63 MiB），
  保留于 `runtime/gpu-export/licenses/` 供续跑复用；其中 18 个源码归档已收集，不能等同于发行审查通过。
- zlib 源码下载从官网及官方 GitHub 均遇到 TLS 连接错误；未生成最终 `materials.json`。
  发行材料审查仍未完成，`packaging_license_review_complete` 保持 false，未更换 PyAV/GPU 二进制或绕过检查。
- 本轮测试临时目录已清理，任务目录剩余小型记录约 30 KiB；材料保留至 2026-09-26 复核。
  未复制模型或开发环境，未登记 v2.3.0 更新基线，尚未执行新包 `--check-upload` 验收。
- 详细状态与续跑入口见 [v2.3.0 发行准备记录](V230_READINESS_20260919.md)，用户文案见
