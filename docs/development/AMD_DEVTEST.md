...
```powershell
cmd /c native\amd_probe\build.bat
.venv\Scripts\python.exe -m unittest tests.test_amd_devtest -v
.venv\Scripts\python.exe -m dlss5tool.amd_devtest --inventory --root tmp\amd-devtest-local
# NVIDIA 机器仅验证原生/FSR 基线，不加载 AMD 模组：
.venv\Scripts\python.exe -m dlss5tool.amd_devtest --baseline-only --adapter 0 --root tmp\amd-devtest-baseline
powershell -ExecutionPolicy Bypass -File scripts/build_amd_devtest.ps1
```
