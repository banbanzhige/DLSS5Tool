...
1. 先将两条已证明可行的编码前直连，补到与应用**相同**的编码配置和颜色转换，再做连续真实光流、
   ...
   **2026-09-16 补测：** [编码验收](ENCODE_ACCEPTANCE_20260916.md) 已跑完。实验 CONSTQP
   搬运仍成立；单缓冲原生 HQ VBR/CQ 与生产 `h264_nvenc p5 hq vbr cq19` 解码画面不一致，
   **不得接入默认路径**。2026-09-16 环缓冲补测见
   [RING.md](encode-acceptance-20260916/RING.md)：P5 HQ 为 3 个 B 帧、lookahead 关；
   系统内存 YV12 仍与生产解码 MD5 不一致。`format=nv12` 也不能代替 `format=yuv420p`。
2. VSR 先定位 4K 差异；HDR/深度先解决数值合同，不将算子测速变成画质通过。
   HDR 分析候选在 CPU 上与生产 `analysis_rgba8` 逐位相同；混合仍有半精度误差。
