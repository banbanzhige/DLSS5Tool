...
- **实时预览与直观对比**：原图、处理后和可拖动分界线对比随参数即时更新；预览支持以鼠标位置为中心缩放、拖动平移和导航小窗，还可连同整组播放控件分离到自由缩放的独立窗口，方便在导出前检查局部细节或使用双显示器调参。
...
- 视频暂停、拖动或跳转后，会在「拖动后生成」设定的延迟结束时自动从当前帧向后预渲染，无需先按播放。导入、拖动时间轴、缩放和平移时会先冻结后台缓存，优先保证画面跟手，交互结束后再恢复预渲染。
...
- 设置校验、原子写入、损坏后自动恢复。
- 首次运行采用推荐预设：全部标准处理项启用，原始分辨率预览、8192 MiB 缓存、均衡画质、p7 编码、严格时序与 3 帧 GPU 队列；已有配置保持不变。
- 主界面提供「一键诊断」：自动核对 GPU/驱动、DLSSNR 与宿主 DLL、当前设置，并分别运行 v2/旧宿主单帧探针，导出可直接发送给维护者的隐私精简日志。
- 免安装版启动后会在后台检查项目 GitHub Releases；发现更高的正式版本时提示更新，并可在用户确认后自动下载、校验完整的 win64 便携包。
...
下载 `DLSS5Tool-v1.2.2-win64.zip` 并完整解压，然后双击
...
免安装版每次启动时会在后台检查一次最新正式 Release，也可随时点击主界面右侧的「检查更新」。
已经是最新版、本地版本更高或自动检查遇到网络错误时均不弹窗，只有发现真正的新版本才提示。
发现新版本后，程序只会在用户确认时把完整 win64 ZIP 下载到系统“下载”文件夹；下载时使用
临时文件，并校验 Release 标注的文件大小和 SHA-256（如 GitHub 提供）。程序不会在运行中覆盖
自身，请关闭旧版本、完整解压下载的 ZIP 后再运行新版。源码运行默认不做启动检查，但按钮仍可用；
如需关闭免安装版的启动检查，可设置环境变量 `DLSS5TOOL_DISABLE_UPDATE_CHECK=1`。
...
- 预览画布内滚轮缩放，放大后拖动平移；右下角导航小窗可点击或拖动定位，`0` 恢复适应窗口，`+` / `-` 调整倍率。
- `Space` 播放/暂停，方向键逐帧，`Shift` + 方向键跳 1 秒；时间轴上的滚轮仍可逐帧移动。
- 点击「分离预览」可把画面、时间轴和播放控件整体移入独立窗口；窗口可缩至 `320×240`，底部控件会自动换行，便于让窗口比例贴合竖屏素材并减少黑边。关闭独立窗口或点击「停靠主窗」会恢复原布局，窗口位置、大小和分离状态自动记忆。
...
- 在「预览与调参」Tab 点击「加入队列」，可把当前视频或图片和当前参数快照加入批量任务；之后继续调参不会改变已排队任务。
- 在「导出队列」Tab 可混合添加图片与视频，或递归添加文件夹。输出目录留空时写到各源文件旁边，已有同名文件会自动使用编号后缀。
- 队列逐个处理媒体；图片保持源图片格式、原尺寸和 SDR 路径，视频可使用 MP4/MKV/MOV 容器，单个 SDR 视频仍可并行分段，HDR 任务自动使用严格单会话。
- 遇到初始化、DLL 或显卡兼容问题时，无需先导入素材，点击主界面右侧「一键诊断」并保存 `.log`，再把该文件发送给维护者即可。
